...
A dependency-free tool for KYC / case-review work that matches names,
emails, phones, and addresses across accounts, including previously used
contact information -- name variants, matching emails, matching phone
numbers, matching addresses -- a common signal for duplicate accounts,
synthetic identities, or an account takeover where the attacker recycles
the victim's previously used contact info on a second account.
...
def compare_accounts(account_a: dict, account_b: dict, *, is_org: bool = False) -> PIICollision:
    ...
    for field_name, normalize in PII_FIELDS.items():
        ...
        if val_a and val_b and normalize(val_a) == normalize(val_b):
            ...
            reasons.append(f"same {field_name} on both accounts")
    ...
    for field_name, prior_key in (("email", "priorEmails"), ("phone", "priorPhones")):
        ...
        if val_b and normalize(val_b) in prior_a:
            ...
            reasons.append(
                f"{name_b}'s {field_name} matches {name_a}'s retired {field_name}"
            )
        if val_a and normalize(val_a) in prior_b:
            field_matches.append(f"{field_name} (retired)")
            reasons.append(
                f"{name_a}'s {field_name} matches {name_b}'s retired {field_name}"
            )
...
def load_accounts(path: str) -> list[dict]:
    ...
    if p.is_dir():
        account_files = sorted(p.glob("account-data*.json"))
        if not account_files:
            raise ValueError(f"no account-data*.json files found in directory: {path}")
        records = []
        for account_file in account_files:
            records.extend(load_accounts(str(account_file)))
        return records
...
def print_collision(collision: PIICollision) -> None:
    ...
    acct_masked = collision.account_b.get("accountNumberMasked")
    if acct_masked:
        print(f"  associated account: {acct_masked}")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--accounts", dest="accounts_path", nargs="+", help="one or more JSON files, or a simulated-account directory, to scan for PII collisions across accounts")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.accounts_path:
        try:
            accounts = []
            for accounts_path in args.accounts_path:
                accounts.extend(load_accounts(accounts_path))
